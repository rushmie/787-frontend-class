...
var namesMap = names.map((n) => speakName(n));
...
function nameReducer(accumulator, name) {
  var firstLetter = name.charAt(0).toLowerCase();
  if (firstLetter === 'j') {
...
var namesReduced = names.reduce(nameReducer, initialValue);
for(var n in namesReduced.hello){
  console.log(namesReduced.hello[n]);
}
for(var n in namesReduced.bye){
  console.log(namesReduced.bye[n]);
}
